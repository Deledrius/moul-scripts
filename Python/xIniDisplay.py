    ...
    if gFilenameAndPath == "":
        if PtIsInternalRelease():
            ...
            if PtFileExists(localNameAndPath):
                ...
                print("xIniDisplay::ConstructFilenameAndPath(): Using internal \"" + gFilenameAndPath + "\" file")
                return
        # otherwise, use the standard init path
        gFilenameAndPath = PtGetInitPath() + "/" + gFilename
        print("xIniDisplay::ConstructFilenameAndPath(): Using user-level \"" + gFilenameAndPath + "\" file")
