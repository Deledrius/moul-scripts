...
class nb01EmgrPhase0(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        for sdlVar in list(self.SpecialPagingVars.keys()):
            ageSDL.setNotify(self.key, sdlVar, 0.0)
        ...
        if self.sceneobject.isLocallyOwned() and ageSDL["nb01StainedGlassGZ"][0] != 0:
    ...
    def IManageLinkPedestal(self, position):
        ...
        if position in self.Pedestals:
            ...
            if state:
                if state in pedInfo:
                    # We have this book...
                    if self._pedestalSDLValues[pedInfo["sdlGlass"]] not in pedInfo[state]["Glasses"]:
                        # ...but the glass is wrong.
                        self.IPickPedestalGlass(position, pedInfo[state])
                else:
    ...
    def IPickPedestalBook(self, position):
        if position in self.Pedestals:
            ...
            ageSDL = PtGetAgeSDL()
            ageSDL[self.Pedestals[position]["sdlState"]] = (newBookChoice, )
    ...
    def IPickPedestalGlass(self, position, bookInfo):
        if position in self.Pedestals:
            ...
            ageSDL = PtGetAgeSDL()
            PtDebugPrint("nb01EmgrPhase0.IPickPedestalGlass():\tWas {}.".format(ageSDL[self.Pedestals[position]["sdlGlass"]]))
            ageSDL[self.Pedestals[position]["sdlGlass"]] = (newGlass, )
        else:
    ...
    def _RandomizeNeighborhood(self):
        ...
        for name, values in self.HoodDecorations.iteritems():
            ...
            if var is None:
                PtDebugPrint("nb01EmgrPhase0._RandomizeNeighborhood(): '{}' not found!".format(name))
                continue
            # Duck Typing makes this unnecessary.
            if var.getType() == PtSDLVarType.kBool:
                ...
                PtDebugPrint("nb01EmgrPhase0._RandomizeNeighborhood(): '{}' has an unknown type! Not randomizing.".format(name))
                continue
            ...
            if not var.isUsed():
                ...
                func(theChosenValue)
                PtDebugPrint("nb01EmgrPhase0._RandomizeNeighborhood(): '{}' = {}".format(name, theChosenValue))
        PtDebugPrint("nb01EmgrPhase0._RandomizeNeighborhood(): ---SDL Randomized!---")
        vault.updateAgeSDL(ageSDL)


    # Simple Paging function
